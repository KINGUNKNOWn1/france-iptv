...
const LGTV = () => {
  ...
  const howToSchema = {
    ...
    '@type': 'HowTo',
    name: "Installer l'IPTV sur LG Smart TV",
    description: "Guide étape par étape pour installer l'IPTV sur votre LG Smart TV sous WebOS. Fonctionne pour les modèles OLED et NanoCell récents.",
    totalTime: 'PT5M',
    tool: [
      { '@type': 'HowToTool', name: 'LG Smart TV (WebOS)' },
      { '@type': 'HowToTool', name: 'Connexion internet' },
      { '@type': 'HowToTool', name: 'Abonnement IPTV France IPTV' }
    ],
    step: [
      { '@type': 'HowToStep', position: 1, name: 'Ouvrez le LG Content Store', text: "Depuis la Home LG, ouvrez le LG Content Store (icône boutique)." },
      { '@type': 'HowToStep', position: 2, name: "Recherchez une application IPTV", text: "Recherchez « IPTV Smarters » dans le Content Store." },
      { '@type': 'HowToStep', position: 3, name: "Installez l'application", text: "Installez et ouvrez l'application. Elle affiche une adresse MAC ou demande vos identifiants." },
      { '@type': 'HowToStep', position: 4, name: 'Activez votre abonnement', text: "Contactez-nous via WhatsApp avec votre adresse MAC ou vos identifiants. Activation en moins d'une minute." },
      { '@type': 'HowToStep', position: 5, name: 'Commencez à regarder', text: "Redémarrez l'application : votre abonnement France IPTV est actif." }
    ]
  };
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: "L'IPTV fonctionne-t-il sur toutes les LG Smart TV ?",
        acceptedAnswer: { '@type': 'Answer', text: "L'IPTV fonctionne sur la plupart des LG Smart TV sous WebOS 3.0 et supérieur (modèles depuis 2016 environ). Le LG Content Store propose un choix d'applications IPTV plus limité que le Samsung App Store : si aucune application adaptée n'y figure, un boîtier Android TV ou un Amazon Fire Stick reste la solution la plus simple." }
      },
      {
        '@type': 'Question',
        name: "Quelle application utiliser sur LG Smart TV ?",
        acceptedAnswer: { '@type': 'Answer', text: "« IPTV Smarters » est généralement disponible dans le LG Content Store. Si elle n'apparaît pas pour votre modèle ou région, un Amazon Fire Stick ou un boîtier Android TV connecté en HDMI offre un choix d'applications bien plus large (IPTV Smarters Pro, TiviMate, GSE Smart IPTV)." }
      },
      {
        '@type': 'Question',
        name: "Le LG Content Store ne propose aucune application IPTV",
        acceptedAnswer: { '@type': 'Answer', text: "C'est fréquent sur certains modèles ou versions de WebOS plus anciennes. La solution la plus fiable est de connecter un Amazon Fire Stick ou un boîtier Android TV sur un port HDMI de votre LG TV : vous accédez alors à un catalogue d'applications bien plus large." }
      },
      {
        '@type': 'Question',
        name: "Mon image saccade ou l'application plante sur ma LG TV",
        acceptedAnswer: { '@type': 'Answer', text: "Vérifiez votre débit internet (10 Mbps minimum HD, 25 Mbps 4K). Privilégiez une connexion Ethernet au Wi-Fi. Mettez à jour WebOS depuis Paramètres > Assistance > Mise à jour logicielle. Si l'application continue de planter, un boîtier externe (Fire Stick, Android TV) contourne le problème." }
      },
      {
        '@type': 'Question',
        name: "Combien de temps prend l'activation ?",
        acceptedAnswer: { '@type': 'Answer', text: "Comptez environ 5 minutes entre votre commande et la réception de vos identifiants ou l'activation de votre adresse MAC via WhatsApp." }
      }
    ]
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'LG Smart TV', item: 'https://franceiptv.stream/appareils/lg-tv' }
    ]
  };
  ...
  const webPageSchema = {
    '@context': 'https://schema.org',
    '@type': 'WebPage',
    name: "Installer l'IPTV sur LG Smart TV - Guide 2026",
    description: "Installez l'IPTV sur votre LG Smart TV (WebOS, OLED, NanoCell) en 5 minutes.",
    url: 'https://franceiptv.stream/appareils/lg-tv'
  };

  const compatibleModels = [
    { series: 'LG OLED (WebOS 4.0+)', supported: true },
    { series: 'LG NanoCell (WebOS 4.0+)', supported: true },
    { series: 'LG UHD (WebOS 3.5+)', supported: true },
    { series: 'LG Smart TV WebOS 3.0', supported: true, note: 'Choix d\'applications plus limité' },
    { series: 'Anciens modèles LG (pré-WebOS)', supported: false, alternative: 'Utilisez un boîtier Android TV ou Amazon Fire Stick' }
  ];

  const installationSteps = [
    { number: 1, title: 'Ouvrez le LG Content Store', description: "Depuis la Home LG, ouvrez le LG Content Store (icône boutique).", icon: Tv, time: '30 secondes', difficulty: 'Facile' },
    { number: 2, title: 'Recherchez une application IPTV', description: 'Recherchez « IPTV Smarters » dans le Content Store.', icon: Download, time: '1 minute', difficulty: 'Facile' },
    { number: 3, title: "Installez l'application", description: "Sélectionnez « Installer » et attendez la fin du téléchargement.", icon: Settings, time: '2 minutes', difficulty: 'Facile' },
    { number: 4, title: "Ouvrez & notez l'adresse MAC", description: "Ouvrez l'application : une adresse MAC ou un écran de connexion s'affiche.", icon: Smartphone, time: '30 secondes', difficulty: 'Facile' },
    { number: 5, title: 'Activez via WhatsApp', description: "Envoyez-nous votre adresse MAC ou vos identifiants via WhatsApp. Activation en moins d'une minute.", icon: Phone, time: '1 minute', difficulty: 'Facile' },
    { number: 6, title: 'Commencez à regarder', description: "Redémarrez l'application : profitez de 30 500+ chaînes en HD et 4K.", icon: Play, time: '30 secondes', difficulty: 'Facile' }
  ];

  const troubleshooting = [
    {
      problem: "Aucune application IPTV disponible dans le LG Content Store",
      solution: "Le catalogue LG est plus restreint que celui de Samsung. Connectez un Amazon Fire Stick ou un boîtier Android TV sur un port HDMI de votre TV pour accéder à un choix d'applications bien plus large.",
      icon: AlertCircle
    },
    {
      problem: 'Image qui saccade ou mémoire tampon',
      solution: "Vérifiez votre débit internet (10 Mbps minimum HD, 25 Mbps 4K). Connectez votre TV en Ethernet plutôt qu'en Wi-Fi. Redémarrez votre routeur si besoin.",
      icon: Wifi
    },
    {
      problem: "L'application plante ou se ferme automatiquement",
      solution: "Mettez à jour WebOS depuis Paramètres > Assistance > Mise à jour logicielle. Supprimez et réinstallez l'application IPTV. Si le problème persiste, un boîtier externe (Fire Stick, Android TV) est plus fiable sur certains modèles.",
      icon: Zap
    },
    {
      problem: "L'adresse MAC ou mes identifiants ne sont pas acceptés",
      solution: "Vérifiez que vous avez transmis la bonne information sans espace superflu. Contactez-nous via WhatsApp si le problème persiste.",
      icon: Shield
    }
  ];

  const benefits = [
    { icon: Zap, title: 'Installation Rapide', description: 'Un IPTV fonctionnel sur votre LG TV en 5 minutes' },
    { icon: Tv, title: 'OLED & NanoCell', description: 'Compatible avec les gammes LG OLED et NanoCell récentes' },
  ...
  return (
    <>
      <Helmet>
        <title>Installer l'IPTV sur LG Smart TV (WebOS) 2026 | France IPTV</title>
        <meta name="description" content="Installer l'IPTV sur LG Smart TV en 5 minutes ✓ WebOS, OLED, NanoCell ✓ Guide étape par étape ✓ Support francophone." />
        <meta name="keywords" content="iptv lg tv, iptv lg smart tv, iptv webos, installer iptv lg, application iptv lg" />
        <link rel="canonical" href="https://franceiptv.stream/appareils/lg-tv" />

        <meta property="og:title" content="Installer l'IPTV sur LG Smart TV - Guide Complet 2026" />
        <meta property="og:description" content="Installez l'IPTV sur votre LG Smart TV en 5 minutes. WebOS, OLED, NanoCell." />
        <meta property="og:url" content="https://franceiptv.stream/appareils/lg-tv" />
        <meta property="og:type" content="article" />

        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Installer l'IPTV sur LG Smart TV" />
        <meta name="twitter:description" content="Installation en 5 minutes ✓ WebOS, OLED, NanoCell" />
        ...
        <div className="relative z-10 container mx-auto px-4 py-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center text-white max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-6 py-2 mb-8">
              ...
              <span className="font-medium">WebOS · OLED · NanoCell</span>
            </motion.div>

            <h1 className="text-5xl md:text-6xl font-bold mb-6 leading-tight">
              Installer l'IPTV sur
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 mt-2">LG Smart TV</span>
            </h1>

            <p className="text-xl md:text-2xl mb-8 text-gray-200 max-w-3xl mx-auto leading-relaxed">
              Installez l'IPTV France IPTV en <span className="font-bold text-blue-400">5 minutes</span> sur votre LG Smart TV.
            </p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="https://wa.me/18653169315?text=Bonjour%2C%20je%20souhaite%20installer%20l%27IPTV%20sur%20ma%20LG%20TV" className="bg-brand-gold text-white px-8 py-4 rounded-full font-semibold text-lg hover:shadow-2xl hover:shadow-blue-500/50 transition-all duration-300 flex items-center justify-center gap-2">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-xl text-brand-gray max-w-2xl mx-auto">Suivez ces étapes et regardez l'IPTV sur votre LG TV en 5 minutes</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-12">
            ...
            <a href="https://wa.me/18653169315?text=Bonjour%2C%20je%20souhaite%20commander%20IPTV%20pour%20ma%20LG%20TV" className="inline-flex items-center gap-2 bg-brand-gold text-white px-8 py-4 rounded-full font-semibold text-lg hover:shadow-xl transition-all">
      ...
      <section className="py-20 bg-gradient-to-br from-[#141311] to-[#201C18] text-white">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold mb-4">Modèles LG <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">Compatibles</span></h2>
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">L'IPTV France IPTV fonctionne sur la plupart des LG Smart TV</p>
          </motion.div>
        ...
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16 relative">
            ...
            <div className="relative z-10">
              ...
              <p className="text-xl text-brand-gray max-w-2xl mx-auto">Problèmes courants sur LG Smart TV et leurs solutions</p>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mt-12">
            ...
            <a href="https://wa.me/18653169315?text=Bonjour%2C%20j%27ai%20un%20probl%C3%A8me%20avec%20l%27IPTV%20sur%20ma%20LG%20TV" className="inline-flex items-center gap-2 bg-brand-gold text-white px-8 py-3 rounded-full font-semibold hover:shadow-xl transition-all">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-xl text-brand-gray">Tout ce que vous devez savoir sur l'IPTV sur LG Smart TV</p>
          </motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white">
        <div className="container mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto">Installez l'IPTV France IPTV sur votre LG Smart TV en 5 minutes.</p>
            <a href="https://wa.me/18653169315?text=Bonjour%2C%20je%20souhaite%20commander%20IPTV%20pour%20ma%20LG%20TV" className="inline-flex items-center gap-2 bg-white text-blue-600 px-10 py-5 rounded-full font-bold text-xl hover:shadow-2xl transition-all">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-6 text-sm">
              ...
              <div className="flex items-center gap-2"><Check className="w-5 h-5" /><span>OLED & NanoCell</span></div>
